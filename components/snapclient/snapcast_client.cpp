...
void SnapcastClient::notify_audio_played_tagged(uint32_t frames, int64_t adjusted_ts, const audio::RenderTag &tag) {
  ...
  if (err_tag_valid &&
      first_frame_local >= this->dl_blank_until_us_) {
    if (this->dl_acc_n_ == 0) {
...
void SnapcastClient::player_task_() {
  ...
  while (!this->shutdown_.load(std::memory_order_relaxed)) {
    ...
    // ── ACTUATORS, ONCE PER CHUNK, UNCONDITIONALLY ────────────────────────────────────────────
    // Both writes used to live inside branches of the ladder: the position apply inside the
    // coarse-window-step branch, and set_trim_ppm two levels inside the median-window branch. So
    // a command was only delivered when the old ladder would itself have acted there, and the
    // engine's decisions were silently dropped everywhere else -- it would set in_flight_, wait
    // for a landing that never came, and retry three horizons later. Decide in one place, apply
    // in one place.
#ifdef USE_I2S_RATE_LOCK
    if (this->rate_lock_ != nullptr && st.rate_lock_ok) {
      st.trim_applied_ppm = engine_cmd.rate_ppm;
      if (!this->rate_lock_->set_trim_ppm(st.trim_applied_ppm)) {
        st.rate_lock_ok = false;
        ESP_LOGW(TAG, "Rate lock unavailable, position corrections only");
      }
    ...
    if (applied_frames > 0) {
      ...
      st.soft_dropped_frames += static_cast<uint32_t>(applied_frames);
    } else if (applied_frames < 0) {
      st.soft_inserted_frames += static_cast<uint32_t>(-applied_frames);
      this->push_silence_(static_cast<uint32_t>(-applied_frames), rec.params);
    }
    // A delivered correction moves the audio, so the phase does not describe it until the
    // horizon passes. Same reason the old path armed this; kept because it is client policy.
    if (applied_frames != 0) {
      st.phase_transient_until_us =
          std::max(st.phase_transient_until_us, now_us() + this->visibility_horizon_us());
...
timing::Command SnapcastClient::timing_step_(ServoState &st, uint32_t sample_rate,
  ...
  if (cmd.frames != 0 || tnow - this->timing_log_us_ >= 2000000) {
    ...
    ESP_LOGD(TAG,
             ...
             "sigma=%.1f gsig=%s esrc=%c ep=%+" PRId64 " gate=%" PRId64 " sup=%" PRIu32
             " t=%" PRId64,
             ...
             gsig_buf, cmd.decision.e_from_diff ? 'd' : 'l', cmd.decision.e_position_us,
             cmd.decision.gate_us, cmd.decision.suppressed, tnow);
